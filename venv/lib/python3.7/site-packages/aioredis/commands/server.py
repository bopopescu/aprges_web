...
class ServerCommandsMixin:
    ...
    def subordinateof(self, host=_NOTSET, port=None):
        """Make the server a subordinate of another instance,
        or promote it as main.

        Calling ``subordinateof(None)`` will send ``SLAVEOF NO ONE``.

        .. versionchanged:: v0.2.6
           ``subordinateof()`` form deprecated
           in favour of explicit ``subordinateof(None)``.
        """
        if host is _NOTSET:
            logger.warning("subordinateof() form is deprecated!"
                           " Use subordinateof(None) to turn redis into a MASTER.")
            host = None
            # TODO: drop in 0.3.0
...
MainInfo = namedtuple('MainInfo', 'role replication_offset subordinates')
MainSubordinateInfo = namedtuple('MainSubordinateInfo', 'ip port ack_offset')

SubordinateInfo = namedtuple('SubordinateInfo',
                       'role main_ip main_port state received')

SentinelInfo = namedtuple('SentinelInfo', 'role mains')


def parse_role(role):
    type_ = role[0]
    if type_ == 'main':
        subordinates = [MainSubordinateInfo(s[0], int(s[1]), int(s[2]))
                  for s in role[2]]
        return MainInfo(role[0], int(role[1]), subordinates)
    elif type_ == 'subordinate':
        return SubordinateInfo(role[0], role[1], int(role[2]), role[3], int(role[4]))
    elif type_ == 'sentinel':
