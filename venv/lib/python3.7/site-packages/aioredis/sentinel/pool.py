...
from ..errors import (
    MainNotFoundError,
    SubordinateNotFoundError,
    PoolClosedError,
    RedisError,
    MainReplyError,
    SubordinateReplyError,
)
...
class SentinelPool:
    ...
    def __init__(self, sentinels, *, db=None, password=None, ssl=None,
        ...
        #       and what to do if no main is found:
        ...
        async def echo_events():
            try:
                ...
                #   freeze reconnection
                #   wait / discover new main (find proper way)
                #   unfreeze reconnection
                #
                #   discover main in default way
                #       get-main-addr...
                #       connnect
                #       role
                #       etc...
    ...
    def discover_timeout(self):
        ...
        main/subordinate address discovery.
        """
        return self._timeout

    def main_for(self, service):
        """Returns wrapper to main's pool for requested service."""
        # TODO: make it coroutine and connect minsize connections
        if service not in self._mains:
            self._mains[service] = ManagedPool(
                self, service, is_main=True,
                db=self._redis_db,
                ...
                loop=self._loop)
        return self._mains[service]

    def subordinate_for(self, service):
        """Returns wrapper to subordinate's pool for requested service."""
        # TODO: make it coroutine and connect minsize connections
        if service not in self._subordinates:
            self._subordinates[service] = ManagedPool(
                self, service, is_main=False,
                db=self._redis_db,
                password=self._redis_password,
                encoding=self._redis_encoding,
                minsize=self._redis_minsize,
                ...
                parser=self._parser_class,
                loop=self._loop)
        return self._subordinates[service]
    ...
    async def _do_close(self):
        ...
        while self._mains:
            _, pool = self._mains.popitem()
            pool.close()
            tasks.append(pool.wait_closed())
        while self._subordinates:
            _, pool = self._subordinates.popitem()
            pool.close()
            tasks.append(pool.wait_closed())
    ...
    async def discover(self, timeout=None):    # TODO: better name?
        ...
        If not all monitored services (mains/subordinates) discovered
    ...
    async def discover_main(self, service, timeout):
        """Perform Main discovery for specified service."""
        # TODO: get lock
        ...
        for sentinel in pools:
            try:
                with async_timeout(timeout, loop=self._loop):
                    address = await self._get_mains_address(
                        sentinel, service)

                pool = self._mains[service]
                with async_timeout(timeout, loop=self._loop), \
                        contextlib.ExitStack() as stack:
                    conn = await pool._create_new_connection(address)
                    stack.callback(conn.close)
                    await self._verify_service_role(conn, 'main')
                    stack.pop_all()
                ...
                raise MainReplyError("Service {} error".format(service), err)
            except Exception:
                # TODO: clear (drop) connections to schedule reconnect
                await asyncio.sleep(idle_timeout, loop=self._loop)
                continue
        else:
            raise MainNotFoundError("No main found for {}".format(service))

    async def discover_subordinate(self, service, timeout, **kwargs):
        """Perform Subordinate discovery for specified service."""
        # TODO: use kwargs to change how subordinates are picked up
        ...
        for sentinel in pools:
            try:
                with async_timeout(timeout, loop=self._loop):
                    address = await self._get_subordinate_address(
                        sentinel, service)  # add **kwargs
                pool = self._subordinates[service]
                with async_timeout(timeout, loop=self._loop), \
                        contextlib.ExitStack() as stack:
                    conn = await pool._create_new_connection(address)
                    stack.callback(conn.close)
                    await self._verify_service_role(conn, 'subordinate')
                    stack.pop_all()
                return conn
            except asyncio.CancelledError:
                raise
                ...
                raise SubordinateReplyError("Service {} error".format(service), err)
            except Exception:
                await asyncio.sleep(idle_timeout, loop=self._loop)
                continue
        raise SubordinateNotFoundError("No subordinate found for {}".format(service))

    async def _get_mains_address(self, sentinel, service):
        # NOTE: we don't use `get-main-addr-by-name`
        #   as it can provide stale data so we repeat
        #   after redis-py and check service flags.
        state = await sentinel.execute(b'sentinel', b'main',
    ...
    async def _get_subordinate_address(self, sentinel, service):
        # Find and return single subordinate address
        subordinates = await sentinel.execute(b'sentinel', b'subordinates',
                                        service, encoding='utf-8')
        if not subordinates:
            raise UnknownService()
        for state in map(make_dict, subordinates):
            address = state['ip'], int(state['port'])
            flags = set(state['flags'].split(','))
            if {'s_down', 'o_down', 'disconnected'} & flags:
                continue
    ...
    def _need_rediscover(self, service):
        ...
        pool = self._mains.get(service)
        if pool:
            pool.need_rediscover()
        pool = self._subordinates.get(service)
        if pool:
            pool.need_rediscover()
...
class ManagedPool(ConnectionsPool):
    ...
    def __init__(self, sentinel, service, is_main,
    ...
    async def _create_new_connection(self, address):
        if address is _NON_DISCOVERED:
            ...
            if self._is_main:
                conn = await self._sentinel.discover_main(
                    self._service, timeout=self._sentinel.discover_timeout)
            else:
                conn = await self._sentinel.discover_subordinate(
                    self._service, timeout=self._sentinel.discover_timeout)
...
class DiscoverError(Exception):
    """Internal errors for mains/subordinates discovery."""


class BadState(DiscoverError):
    """Bad main's / subordinate's state read from sentinel."""
