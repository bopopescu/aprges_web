...
class RedisSentinel:
    ...
    def main_for(self, name):
        """Returns Redis client to main Redis server."""
        # TODO: make class configurable
        return Redis(self._pool.main_for(name))

    def subordinate_for(self, name):
        """Returns Redis client to subordinate Redis server."""
        # TODO: make class configurable
        return Redis(self._pool.subordinate_for(name))
    ...
    def main(self, name):
        ...
        return wait_convert(fut, parse_sentinel_main)

    def main_address(self, name):
        """Returns a (host, port) pair for the given ``name``."""
        fut = self.execute(b'get-main-addr-by-name', name, encoding='utf-8')
    ...
    def mains(self):
        ...
        # TODO: process mains: we can adjust internal state
        return wait_convert(fut, parse_sentinel_mains)

    def subordinates(self, name):
        """Returns a list of subordinates for ``name``."""
        fut = self.execute(b'SLAVES', name, encoding='utf-8')
        return wait_convert(fut, parse_sentinel_subordinates_and_sentinels)
    ...
    def failover(self, name):
        """Force a failover of a named main."""
    ...
    def check_quorum(self, name):
        ...
        to reach the quorum needed to failover a main,
